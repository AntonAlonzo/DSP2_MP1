...
for i = 1:length(comped)
   ...
   if mag >= 0 && mag <= 2/8 % segments 0 & 1
       exped(i) = comped(i);
   elseif mag > 2/8 && mag <= 3/8 % segment 2
       exped(i) = ceil(mag*2)*s; % 2:1 compression
   elseif mag > 3/8  && mag <= 4/8 % segment 3
       exped(i) = ceil(mag*4)*s; % 4:1 compression
   elseif mag > 4/8 && mag <= 5/8 % segment 4
       exped(i) = ceil(mag*8)*s; % 8:1 compression
   elseif mag > 5/8 && mag <= 6/8 % segment 5
       exped(i) = ceil(mag*16)*s; % 16:1 compression
   elseif mag > 6/8 && mag <= 7/8 % segment 6
       exped(i) = ceil(mag*32)*s; % 32:1 compression
   elseif mag > 7/8 && mag <= 1 % segment 7
       exped(i) = ceil(mag*64)*s; % 64:1 compression
   end
end
